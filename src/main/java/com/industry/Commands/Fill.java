...
public class Fill {
    ...
    private static int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        List<String> argNames = List.of("blockState1", "blockState2", "blockState3", "blockState4");
        List<BlockState> blockStates = new ArrayList<>();

        for (int i = 1; i <= 4; i++) {
            String name = "blockState" + i;
            try {
                blockStates.add(
                        BlockStateArgumentType.getBlockState(context, name).getBlockState()
                );
